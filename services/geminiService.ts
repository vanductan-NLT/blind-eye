    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          { inlineData: { mimeType: 'image/jpeg', data: cleanBase64 } },
          {
            text: `You are guiding a blind person. Look at this image from their camera.

DESCRIBE what you see and give walking directions in 2-3 SHORT sentences.

Examples of good responses:
- "Table directly ahead, 2 meters. Wall on left. Go right."
- "Clear hallway ahead. Door on the right side."
- "Stop! Person walking toward you from the left."
- "Chair blocking path ahead. Step around to your right."

What do you see? Give directions NOW:` }
        ]
      },
      config: {
        temperature: 0.4,
    ...
    if (!response.text) return "Cannot see clearly.";
    return cleanTextForSpeech(response.text);
